...
const prepareEditPost = (postId, editButton) => {
...
const saveEditPost = async (postId, saveButton, originalContent) => {
    ...
    try {
        ...
        // If successful, update the post element with the new content.
        newTextarea.parentElement.innerHTML = newContent;
    } catch (error) {
        // If the update fails, revert to the original content.
        newTextarea.parentElement.innerHTML = originalContent;
        // Show an alert indicating the update failure.
        alert('Failed to update the post. Error: ' + error.message);
    }
};

// Define the cancelEditPost function to cancel editing and revert to the original content.
const cancelEditPost = (cancelButton, originalContent) => {
